...
# Test update functionality in slash command
test_update_command_structure() {
    test_start "Update functionality in slash command"

    if [ -f ".claude/commands/slash.md" ]; then
        # Check for update subcommand support
        if grep -q "update" ".claude/commands/slash.md" && \
           grep -q "subcommand" ".claude/commands/slash.md" && \
           grep -q "Updating claude-slash commands" ".claude/commands/slash.md"; then
            test_pass "slash.md contains update functionality"
        else
            test_fail "slash.md missing update functionality"
        fi

        # Check for usage documentation
        if grep -q "/slash update" ".claude/commands/slash.md"; then
            test_pass "slash.md documents /slash update usage"
        else
            test_fail "slash.md missing /slash update usage documentation"
        fi
    else
        test_fail "slash.md command file not found"
    fi
}

# Test update logic in slash command (dry run)
test_update_logic() {
    test_start "Update logic in slash command"

    # Extract shell commands from slash.md
    if [ -f ".claude/commands/slash.md" ]; then
        grep "^!" ".claude/commands/slash.md" | sed 's/^!//' > /tmp/test_update_commands.sh 2>/dev/null || true

        if [ -s /tmp/test_update_commands.sh ]; then
            # Test shell syntax
            if bash -n /tmp/test_update_commands.sh 2>/dev/null; then
                test_pass "Slash command shell syntax valid"
            else
                test_fail "Slash command has invalid shell syntax"
            fi

            # Check for GitHub API usage
            if grep -q "api.github.com" /tmp/test_update_commands.sh; then
                test_pass "Slash command uses GitHub API for updates"
            else
                test_fail "Slash command doesn't use GitHub API for updates"
            fi

            # Check for backup functionality
            if grep -q "backup" /tmp/test_update_commands.sh; then
                test_pass "Slash command includes backup functionality"
            else
                test_fail "Slash command missing backup functionality"
            fi
        else
            test_fail "No shell commands found in slash.md"
        fi

        rm -f /tmp/test_update_commands.sh
    else
        test_fail "slash.md not found for logic testing"
    fi
}
...
test_backup_rollback() {
    ...
    local backup_dir=".claude/commands.backup.$(date +%Y%m%d-%H%M%S)"
