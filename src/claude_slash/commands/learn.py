...
This module provides an interactive interface for analyzing current
session context to extract insights and integrate them into CLAUDE.md.
"""
...
from ..ui import SpinnerManager, create_info_table
...
class LearnCommand(BaseCommand):
    ...
    def help_text(self) -> str:
        ...
        return (
            "Continuously refine the global CLAUDE.md file with impactful "
    ...
    def execute(self, **kwargs) -> None:
        ...
        self.console.print(
            Panel.fit("🧠 Interactive Learning Integration System", style="bold blue")
        )
        self.console.print()
        ...
        if not self.claude_md_path.exists():
            ...
            self.console.print(
                "Please ensure your global Claude configuration is set up correctly."
            )
            return
        ...
        if not Confirm.ask(
            ...
            self.console.print(
                "[yellow]❌ Learning extraction cancelled by user.[/yellow]"
            )
            return
    ...
    def _extract_session_learnings(self) -> str:
        ...
        with SpinnerManager.git_operation(
            "🔍 Analyzing current session context for learnings..."
        ):
...
Context: {repo_name}"""
    ...
    def _display_learning_summary(self, learning_content: str) -> None:
        ...
        self.console.print(
            Panel(
                learning_content, title="📊 Extracted Learning Summary", style="green"
            )
        )
        self.console.print()
    ...
    def _parse_claude_md_structure(self) -> List[dict]:
        ...
        for line_num, line in enumerate(lines, 1):
            if line.startswith("#"):
                sections.append(
                    {"line_num": line_num, "header": line.strip(), "type": "existing"}
                )
    ...
    def _interactive_section_selection(self, sections: List[dict]) -> Optional[dict]:
        ...
        if learning_suggestions:
            self.console.print(
                "[green]💡 Suggested sections based on content analysis:[/green]"
            )
    ...
    def _choose_integration_mode(self) -> str:
        ...
        mode_selection = IntPrompt.ask(
            "Select integration mode", choices=["1", "2", "3"], default=1
        )
...
"""
    ...
    def _show_manual_integration(self, section: dict, formatted_learning: str) -> None:
        ...
        self.console.print()
        self.console.print(
            "[yellow]You can manually copy and integrate the learning content above.[/yellow]"
        )
        ...
        self.console.print(
            Panel(
                formatted_learning, title="Learning Content to Integrate", style="green"
            )
        )

    def _integrate_new_section(
        self, section_name: str, formatted_learning: str
    ) -> None:
    ...
    def _integrate_existing_section(
        self, section: dict, formatted_learning: str, mode: str
    ) -> None:
    ...
    def _show_integration_summary(
        self, section: dict, learning_content: str, backup_path: Path
    ) -> None:
        """Show integration summary."""
        self.console.print()
